...
colnames(MainDB)=c("Accesion","ID","Locus","Transcript","Alias","Type")
  ...
  shinyServer(function(input, output, session) {
    ...
    rv <- reactiveValues(
      ...
      basket = data.frame(Plate = c(""), Well = c(""), Spot=c(""), PrimerFwd=c(""), PrimerRev=c(""), crRNA=c(""), Type=c(""), Oligo=c(""), stringsAsFactors=F), 
      ...
      if(wbid == ""){
        ...
        output$SelPiTab <- DT::renderDataTable({
          Pdata=data.frame(
            ...
            crRNA=tmpL[,8],
            Type=tmpL[,9],
            Oligo = shinyInput(actionButton, paste(as.character(tmpL[,1]),"_",as.character(tmpL[,2]),"_",as.character(tmpL[,4]),"_",as.character(tmpL[,5]),"_",as.character(tmpL[,6]),"_",as.character(tmpL[,8]),"_",as.character(tmpL[,10]),sep=""), 'button_', label = "Download genbank", onclick = 'Shiny.onInputChange(\"select_button\",  this.id.concat(\"_\", Math.random()))' ),
            Save = shinyInput(actionButton, paste(as.character(tmpL[,1]),"separator",as.character(tmpL[,2]),"separator",as.character(tmpL[,4]),"separator",as.character(tmpL[,5]),"separator",as.character(tmpL[,6]),"separator",as.character(tmpL[,8]),"separator",as.character(tmpL[,9]),"separator",as.character(tmpL[,10]),sep=""), 'buttonseparator', label = "Add to basket", onclick = 'Shiny.onInputChange(\"add_button2\",  this.id)' ),
    ...
    observeEvent(input$actionmultiplesearch, {
      ...
      if(length(wbidsIN) == 0){
        ...
        output$crRNAMultipleTab <- DT::renderDataTable({
          ...
          colnames(Pdata)[4]="Forward primer"
          colnames(Pdata)[5]="Reverse primer"
          colnames(Pdata)[9]="Annotated oligo sequence (ApE)"
    ...
    observeEvent(input$jsValue, {
      ...
      output$SelPiTabBrowser <- DT::renderDataTable({
        Pdata=data.frame(
          ...
          crRNA=tmpL[,8],
          Type=tmpL[,9],
          Oligo = shinyInput(actionButton, paste(as.character(tmpL[,1]),"_",as.character(tmpL[,2]),"_",as.character(tmpL[,4]),"_",as.character(tmpL[,5]),"_",as.character(tmpL[,6]),"_",as.character(tmpL[,8]),"_",as.character(tmpL[,10]),sep=""), 'button_', label = "Download genbank", onclick = 'Shiny.onInputChange(\"select_button2\",  this.id.concat(\"_\", Math.random()))' ),
          Save = shinyInput(actionButton, paste(as.character(tmpL[,1]),"separator",as.character(tmpL[,2]),"separator",as.character(tmpL[,4]),"separator",as.character(tmpL[,5]),"separator",as.character(tmpL[,6]),"separator",as.character(tmpL[,8]),"separator",as.character(tmpL[,9]),"separator",as.character(tmpL[,10]),sep=""), 'buttonseparator', label = "Add to basket", onclick = 'Shiny.onInputChange(\"add_button2\",  this.id)' ),
          stringsAsFactors = FALSE
        )
    ...
    observeEvent(input$add_button2, {
      ...
      selectedFwdP <- as.character(strsplit(input$add_button2, "separator")[[1]][5])
      selectedRevP <- as.character(strsplit(input$add_button2, "separator")[[1]][6])
      selectedSeq <- as.character(strsplit(input$add_button2, "separator")[[1]][7])
      selectedType <- as.character(strsplit(input$add_button2, "separator")[[1]][8])
      selectedOligo <- as.character(strsplit(input$add_button2, "separator")[[1]][9])
      
      rv$basket <- rbind(rv$basket,c(selectedPlate,selectedWell,selectedSpot,selectedFwdP,selectedRevP,selectedSeq,selectedType,selectedOligo))
    ...
    observeEvent(rv$counter,{
      ...
      output$BigBasket <- DT::renderDataTable({
        ...
        Pdata=data.frame(
          ...
          crRNA=dtt[,6],
          Type=dtt[,7],
          Oligo = shinyInput(actionButton, paste(as.character(dtt[,1]),"_",as.character(dtt[,2]),"_",as.character(dtt[,3]),"_",as.character(dtt[,4]),"_",as.character(dtt[,5]),"_",as.character(dtt[,6]),"_",as.character(dtt[,8]),sep=""), 'button_', label = "Download genbank", onclick = 'Shiny.onInputChange(\"select_button2\",  this.id.concat(\"_\", Math.random()))' ),
    ...
    output$DownBasketApe <- downloadHandler(
      ...
      content = function(file) {
        ...
        for(i in 1:nrow(dtt)){
          tmpLines = OligoApe(dtt[i,8], dtt[i,4], dtt[i,5], dtt[i,6], dtt[i,1], dtt[i,2], dtt[i,3])
